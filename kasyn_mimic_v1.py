...
"""
@author: Xiao Jin
In this file we construct k-async nonlinear part on mimic3 dataset
"""

from functools import partial
from multiprocessing.pool import ThreadPool as Pool
from multiprocessing import Barrier, Lock, BoundedSemaphore
import numpy as np
...
def agg_grads(list_list_gradients):
    m = len(list_list_gradients)  # num of copies
    n = len(list_list_gradients[0])  # num of tensors
    return [tf.add_n([list_list_gradients[j][i] for j in range(m)]) for i in range(n)]
...
class system():
    def __init__(self):
        ...
        """ initialize local workers and models in parallel"""
        ...
        self.optimizer = [tf.keras.optimizers.Adam(learning_rate=learning_rate) for i in range(num_workers)]
        self.optimizer_s = tf.keras.optimizers.Adam(learning_rate=learning_rate*syn_k/num_workers)
        ...
        self.server_gradients = []
        """ k synchronization """
        self.passid = []
        self.syn_k = syn_k
        # self.before_server = Barrier(self.syn_k)
        self.barr1 = Barrier(self.syn_k)
        self.barr2 = Barrier(self.syn_k)
        self.sema1 = BoundedSemaphore(self.syn_k)
        self.sema2 = BoundedSemaphore(self.syn_k)
        self.begin = False

    def kasyn(self, index, train_dataset, test_dataset):
        f = open('kasyn_mimic_lstm_v1' + '.txt', 'w')
        f.close()
        iterations = 0
        # training_loss = 5
        # training_accuracy = 0
        ...
        if index == 0:
            train_loss, train_accuracy, train_M, train_auc = self.test(index, train_dataset, 'vfl_sync_train_roc')
            test_loss, test_accuracy, test_M, test_auc = self.test(index, test_dataset, 'vfl_sync_test_roc')
            print(index, iterations, previous_time, train_loss, train_accuracy, train_auc, test_loss, test_accuracy,
                  test_auc)
            # writing training loss
            f = open('kasyn_mimic_lstm_v1' + '.txt', 'a+')
            f.write(str(index))
            f.write('\t')
        ...
        while previous_time < threshold_time:
            ...
            for k in range(self.nob):
                ...
                self.server_gradients.append(server_gradients)
                self.barr1.wait()
                ''' aggregate gradients from multithreads '''
                if index == self.passid[0]:
                    server_gradients = agg_grads(self.server_gradients)
                    self.server_gradients = []
                    self.optimizer_s.apply_gradients(zip(server_gradients, self.server.trainable_variables))

                self.barr2.wait()
                self.passid.remove(index)
                self.sema1.release()
                ...
                if iterations % 4 == index:
                    # print('loss')
                    train_loss, train_accuracy, train_M, train_auc = self.test(index, train_dataset, 'vfl_kasync_train_roc')
                    test_loss, test_accuracy, test_M, test_auc = self.test(index, test_dataset, 'vfl_kasync_test_roc')
                    print(index, iterations, previous_time, train_loss, train_accuracy, train_auc, test_loss, test_accuracy,
                          test_auc)
                    # writing training loss
                    f = open('kasyn_mimic_lstm_v1' + '.txt', 'a+')
...
if __name__ == "__main__":
    ...
    s = system()
    s.pool.map(partial(s.kasyn, train_dataset=train_dataset, test_dataset=test_dataset), list(range(num_workers)))
    # [0, 1, 2, 3])
