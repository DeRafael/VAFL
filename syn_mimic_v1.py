...
"""
@author: Xiao Jin
In this file we construct sync nonlinear part on mimic3 dataset 
"""
...
class system():
    ...
    def syn(self, index, train_dataset, test_dataset):
        # empty the record txt file
        f = open('syn_mimic_lstm_v1' + '.txt', 'w')
        f.close()
        # roc_file = open('sroc_curve' + '.txt', 'w')
        # roc_file.close()
        ...
        if index == 0:
            ...
            f = open('syn_mimic_lstm_v1' + '.txt', 'a+')
            f.write(str(index))
            f.write('\t')
        ...
        while previous_time < threshold_time:
            if not self.begin:
                continue
            epoch += 1
            """ synchronize sample list """
            self.before_shuffle.wait()
            if index == 0:
                self.idlist = tf.random.shuffle(self.idlist)
            self.after_shuffle.wait()
            running_loss = []
            running_accuracy = []
            # print(self.nob)
            for k in range(self.nob):
                ...
                print(index, k)
                ''' images and labels '''
                batchid = self.idlist[k * batch_size:(k + 1) * batch_size]
                vertical_data = []
                ...
                with tf.device('/job:localhost/replica:0/task:0/device:GPU:0'):
                    with tf.GradientTape(persistent=True) as tape_s:
                        ...
                        ''' server aggregation '''
                        self.before_agg.wait()  # synchronization
                        if index == 0:  # worker 0 is server
                ...
                if index == 0:
                    ...
                    # w_gradients = tape_s.gradient(loss, [self.w])
                ...
                # self.optimizer_s.apply_gradients(zip(worker_gradients, self.workers[index].trainable_variables))
                delay(delay_scale, index)

                end = time()
                previous_time += end - start
            # if index == 0 and iterations % 5 == 0:
            # print(index, iterations, training_loss, training_accuracy)

                if index == 0:  # (iterations / 5) % 4 == index:
                    # print('loss')
                    train_loss, train_accuracy, train_M, train_auc = self.test(index, train_dataset, 'vfl_sync_train_roc')
                    test_loss, test_accuracy, test_M, test_auc = self.test(index, test_dataset, 'vfl_sync_test_roc')
                    print(index, iterations, previous_time, train_loss, train_accuracy, train_auc, test_loss, test_accuracy,
...
if __name__ == "__main__":
    ...
    s = system()
    s.pool.map(partial(s.syn, train_dataset=train_dataset, test_dataset=test_dataset), list(range(num_workers)))
    # [0, 1, 2, 3])
    '''
    # after the training, save the record file based on the current time
    try:
        f1 = open('syn_mimic_lstm_v1' + '.txt', 'r').read()
        f2 = open(str(datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))+'.txt','w')
        f2.write(f1)
        f2.close()
    except:
        print('cannot copy the record')
    '''
